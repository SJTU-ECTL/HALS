...
void NetMan::Compile(double maxDelay) {
    ...
    // synthesize and map
    ostringstream oss("");
    // Some batch-resubstituted candidates can trigger an ABC AIG replacement
    // assertion inside compress2rs. Keep the pre-map normalization minimal; the
    // following dch/map trials still perform technology-oriented cleanup.
    AbcMan::Comm("st; ps;");
    oss.str("");
    oss << maxDelay;
...
bool NetMan::CompileWithYosys(std::string& standCellPath) {
    // Each ResubALS process/invocation gets its own mapping directory.  The
    // historical cwd-relative tmp/aig.blif and tmp/map.v names caused two
    // concurrent kernels to overwrite one another and occasionally read the
    // other run's mapped netlist.
    static atomic<unsigned long long> invocationSerial{0};
    const auto serial = invocationSerial.fetch_add(1, memory_order_relaxed);
    const filesystem::path mappingRoot = filesystem::temp_directory_path()
        / ("resubals_yosys_" + to_string(static_cast<long long>(getpid())));
    filesystem::path mappingDir = mappingRoot / to_string(serial);
    filesystem::create_directories(mappingDir);
    const string aigFileName = (mappingDir / "aig.blif").string();
    const string mapFileName = (mappingDir / "map.v").string();
    ...
    if (useGenlib) {
        ...
        // Check Yosys' own exit status.  Piping through grep made valid
        // constant/empty mappings fail simply because no area line exists.
        auto yosysComm = "yosys -q -p \"" + yosysScript + "\"";
        if (!ExecSystComm(yosysComm)) {
            filesystem::remove_all(mappingDir);
            cerr << "WARNING: optional Yosys mapping failed in " << mappingDir
                 << "; retain the valid ABC-mapped candidate" << endl;
            return false;
        }
    }
    else { // for liberty
    ...
    pNtk = Abc_NtkDup(AbcMan::GetNet()); // NetMan manage the memory of the duplicated network
    filesystem::remove_all(mappingDir);
    // This succeeds only when no other invocation in the process is using the
    // root; failure is harmless and a concurrent invocation removes it later.
    std::error_code cleanupError;
    filesystem::remove(mappingRoot, cleanupError);
    return true;
}
