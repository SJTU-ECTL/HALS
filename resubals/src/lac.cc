...
void LACMan::Gen012ResubLACsPro(NetMan& net, IntVect& nodeIds, unsigned seed, int maxLevelDiff, int nFrame4ResubGen, int maxCandResub) {
    ...
    auto enforceLacLimit = [&]() -> bool {
        if (LAC_NUM_LIMIT <= 0 || static_cast<int>(pLacs.size()) <= LAC_NUM_LIMIT)
            return false;
        sort(pLacs.begin(), pLacs.end(), [](const auto& lhs, const auto& rhs) {
            return dynamic_pointer_cast<ResubLAC>(lhs)->GetSizeGain() >
                   dynamic_pointer_cast<ResubLAC>(rhs)->GetSizeGain();
        });
        pLacs.resize(LAC_NUM_LIMIT);
        cout << "truncated candidate AppResubs to " << pLacs.size() << endl;
        return true;
    };
...
shared_ptr <LAC> LACMan::GetBestResubLacConsiderRealSize(NetMan& net) const {
    ...
    BigFlt bestDeltaDesignErr = numeric_limits <BigFlt>::max();
    ll bestSizeGain = -1;
    for (ll i = 0; i < pLacs.size(); ++i) {
        auto pLac = pLacs[i];
        auto pResubLac = dynamic_pointer_cast <ResubLAC> (pLac);

        BigFlt deltaDesignErr = pLac->GetDeltaDesignErr();
        if (bestDeltaDesignErr > deltaDesignErr || i == 0) {
            bestDeltaDesignErr = deltaDesignErr;
            pBestLac = pLac;
            bestSizeGain = pResubLac->GetSizeGain();
        }
        else if (bestDeltaDesignErr == deltaDesignErr) {
    ...
    cout << "best size gain = " << bestSizeGain
         << ", estimated delta design error = " << bestDeltaDesignErr << endl;
    return pBestLac;
}
