...
private:
    BigInt errBigInt;
    BigFlt deltaDesignErr;
    int targId;

public:
    explicit LAC(): errBigInt(LL_MAX), deltaDesignErr(0), targId(-1) {}
    explicit LAC(int targ_node_id): errBigInt(LL_MAX), deltaDesignErr(0), targId(targ_node_id) {}
    virtual ~LAC() = default;
    ...
    inline BigFlt GetDeltaDesignErr() const {return deltaDesignErr;}
    inline void SetDeltaDesignErr(const BigFlt& delta_design_err) {deltaDesignErr = delta_design_err;}
