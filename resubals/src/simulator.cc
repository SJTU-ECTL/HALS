...
static bool HalsDebugSignedMapeEnabled() {
    const char * env = std::getenv("HALS_DEBUG_SIGNED_MAPE");
    return env != nullptr && std::string(env) != "0";
}

static ll HalsDebugSignedMapeFrameLimit() {
    const char * env = std::getenv("HALS_DEBUG_SIGNED_MAPE_FRAMES");
    if (env == nullptr)
        return 8;
    return std::max<ll>(0, std::atoll(env));
}

static ll HalsDebugSignedMapeFrameStart() {
    const char * env = std::getenv("HALS_DEBUG_SIGNED_MAPE_START");
    if (env == nullptr)
        return 0;
    return std::max<ll>(0, std::atoll(env));
}
...
void Simulator::InpSelf(const string & fileName) {
    ...
    while (fgets(buf, sizeof(buf), fp) != nullptr) {
        if (cnt == nFrame)
            break;
        ll lineLength = static_cast <ll>(strlen(buf));
        bool hasOnlyInputs = lineLength == GetPiNum() + 1;
        bool hasInputsAndOutputs = lineLength == GetPiNum() + GetPoNum() + 1;
        assert(hasOnlyInputs || hasInputsAndOutputs);
        for (ll i = 0; i < GetPiNum(); ++i) {
...
BigInt Simulator::GetOutpRange(ll iPatt, ll lsb, ll msb, bool isSign) const {
    #ifdef DEBUG
    assert(iPatt < nFrame);
    assert(lsb >= 0 && msb < NetMan::GetPoNum());
    assert(lsb <= msb && msb - lsb < 512);
    #endif
    BigInt ret(0);
    for (ll k = msb; k >= lsb; --k) {
...
BigInt Simulator::GetTempOutpRange(ll iPatt, ll lsb, ll msb, bool isSign) const {
    #ifdef DEBUG
    assert(iPatt < nFrame);
    assert(lsb >= 0 && msb < NetMan::GetPoNum());
    assert(lsb <= msb && msb - lsb < 512);
...
double Simulator::GetMeanErrDist(const Simulator & oth_smlt, bool isSign, int outputNum, bool isCheck) const {
    if (isCheck) {
        assert(IsPIOSame(oth_smlt));
    }
    assert(GetPoNum() % outputNum == 0);
    ll width = GetPoNum() / outputNum;
    ...
    for (int i = 0; i < nFrame; ++i) {
        for (int o = 0; o < outputNum; ++o) {
            ll lsb = o * width;
            ll msb = lsb + width - 1;
            auto accOut = GetOutpRange(i, lsb, msb, isSign);
            auto appOut = oth_smlt.GetOutpRange(i, lsb, msb, isSign);
            sed += abs(accOut - appOut);
        }
        // assert(sed < warnPoint);
    }
    auto med = BigFlt(sed) / BigFlt(nFrame * outputNum);
    assert(med < DBL_MAX);
    return (double)(med);
}


double Simulator::GetMeanSquareErr(const Simulator & oth_smlt, bool isSign, int outputNum, bool isCheck) const {
    if (isCheck) {
        assert(IsPIOSame(oth_smlt));
    }
    assert(GetPoNum() % outputNum == 0);
    ...
    for (ll i = 0; i < nFrame; ++i) {
        for (int o = 0; o < outputNum; ++o) {
            ll lsb = o * width;
            ll msb = lsb + width - 1;
            BigInt accOut = GetOutpRange(i, lsb, msb, isSign);
            BigInt appOut = oth_smlt.GetOutpRange(i, lsb, msb, isSign);
            sse += (accOut - appOut) * (accOut - appOut);
        }
    }
    return double(BigFlt(sse) / BigFlt(nFrame * outputNum));
}


double Simulator::GetMeanAbsPercentageErr(const Simulator & oth_smlt, bool isSign, int outputNum, bool isCheck) const {
    auto errors = GetMeanAbsPercentageErrVector(oth_smlt, isSign, outputNum, isCheck);
    BigFlt sum(0);
    for (double error: errors)
        sum += error;
    return double(sum / BigFlt(outputNum));
}


vector<double> Simulator::GetMeanAbsPercentageErrVector(const Simulator & oth_smlt, bool isSign, int outputNum, bool isCheck) const {
    if (isCheck)
        assert(IsPIOSame(oth_smlt));
    assert(GetPoNum() % outputNum == 0);
    ll width = GetPoNum() / outputNum;
    vector<BigFlt> sums(outputNum, BigFlt(0));
    const bool debugSignedMape = isSign && outputNum == 1 && HalsDebugSignedMapeEnabled();
    const ll debugFrameStart = HalsDebugSignedMapeFrameStart();
    const ll debugFrameLimit = HalsDebugSignedMapeFrameLimit();
    for (ll i = 0; i < nFrame; ++i) {
        for (int o = 0; o < outputNum; ++o) {
            ll lsb = o * width;
            ll msb = lsb + width - 1;
            ...
            BigInt appOut = oth_smlt.GetOutpRange(i, lsb, msb, isSign);
            BigFlt localMape(0);
            if (accOut == 0 && appOut == 0) {
                localMape = BigFlt(0);
            }
            else if (accOut != 0) {
                localMape = abs(BigFlt(appOut) - BigFlt(accOut)) / abs(BigFlt(accOut));
                sums[o] += localMape;
            }
            else {
                localMape = abs(BigFlt(1));
                sums[o] += localMape;
            }
            if (debugSignedMape && i >= debugFrameStart && i < debugFrameStart + debugFrameLimit) {
                BigInt dbgA = GetInp(i, 0, 7, true);
                BigInt dbgB = GetInp(i, 8, 15, true);
                cout << "[resubals signed mape] frame=" << i
                     << " a=" << dbgA
                     << " b=" << dbgB
                     << " acc=" << accOut
                     << " app=" << appOut
    ...
    vector<double> errors(outputNum, 0.0);
    for (int o = 0; o < outputNum; ++o)
        errors[o] = double(sums[o] / BigFlt(nFrame));
    return errors;
}
