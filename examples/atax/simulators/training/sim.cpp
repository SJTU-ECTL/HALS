...
static constexpr int widths[]={16,16}, signs[]={0,0};
...
emit({0,0},true,false);
for(int c=0;c<std::max(1,npatterns/3234);++c){
emit({0,0},true,false);
uint64_t x[42]; for(auto &v:x)v=randv(128,512);
for(int i=0;i<38;++i){for(int j=0;j<42;++j){
emit({randv(0,320),x[j]});
}for(int j=0;j<42;++j){
emit({0,0});
}}for(int j=0;j<42;++j){
emit({0,0});
 ...
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/atax_tmp_acc.pattern");
pattern_files.emplace_back(pattern_dir+"/atax_y_acc.pattern");}
...
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<48;++j) pattern_files[1]<<char('0'+((dut.pi1 >> j)&1));pattern_files[1]<<"\n";}
