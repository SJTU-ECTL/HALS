...
static constexpr int widths[]={20}, signs[]={0};
...
uint64_t apply(uint64_t x,Op p,int width){
 ...
 case 4:y=x&p.a;break;case 5:y=x+p.a;break;case 6:y=x>>p.a;break;case 7:y=(x*p.a)/(p.b?p.b:1);break;case 8:y=0;break;}
...
emit({randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255),randv(0,255)});
}
 std::vector<std::vector<uint64_t>> traces(NS);
 std::vector<std::ofstream> pattern_files;
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/hals_conv3x3.pattern");}
 auto simulate=[&](const std::vector<Op>&cfg,bool capture){
  ...
  for(const auto&s:stimuli){dut.a0=s.x[0];
dut.a1=s.x[1];
dut.a2=s.x[2];
dut.a3=s.x[3];
dut.a4=s.x[4];
...
if(capture && !pattern_dir.empty() && cycle<4097){for(int j=0;j<144;++j) pattern_files[0]<<char('0'+((dut.pi0[j/32] >> (j%32))&1));pattern_files[0]<<"\n";}
   if(!s.reset){out.push_back(signvalue(dut.result,20,0));}++cycle;
