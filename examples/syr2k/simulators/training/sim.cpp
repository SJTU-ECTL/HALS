...
static constexpr int widths[]={16}, signs[]={0};
...
emit({randv(0,512),randv(0,512),randv(0,512),randv(0,512),randv(0,512),384,307});
}
 std::vector<std::vector<uint64_t>> traces(NS);
 std::vector<std::ofstream> pattern_files;
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/hals_syr2k.pattern");}
 auto simulate=[&](const std::vector<Op>&cfg,bool capture){
  ...
  for(const auto&s:stimuli){dut.C_in=s.x[0];
dut.A_jk=s.x[1];
dut.B_ik=s.x[2];
dut.B_jk=s.x[3];
dut.A_ik=s.x[4];
...
if(capture && !pattern_dir.empty() && cycle<4097){for(int j=0;j<112;++j) pattern_files[0]<<char('0'+((dut.pi0[j/32] >> (j%32))&1));pattern_files[0]<<"\n";}
   if(!s.reset){out.push_back(signvalue(dut.C_next,16,0));}++cycle;
