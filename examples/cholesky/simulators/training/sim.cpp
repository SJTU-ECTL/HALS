...
if(capture && !pattern_dir.empty() && cycle<4097){for(int j=0;j<48;++j) pattern_files[0]<<char('0'+((dut.pi0 >> j)&1));pattern_files[0]<<"\n";}
if(capture && !pattern_dir.empty() && cycle<4097){for(int j=0;j<80;++j) pattern_files[1]<<char('0'+((dut.pi1[j/32] >> (j%32))&1));pattern_files[1]<<"\n";}
if(capture && !pattern_dir.empty() && cycle<4097){for(int j=0;j<80;++j) pattern_files[2]<<char('0'+((dut.pi2[j/32] >> (j%32))&1));pattern_files[2]<<"\n";}}dut.clk=1;dut.eval();
