...
for(int c=0;c<std::max(1,npatterns/1634);++c){
emit({0,0,0},true,false);
uint64_t p[38];for(auto &v:p)v=randv(128,512);
for(int i=0;i<42;++i){uint64_t r=randv(128,512);for(int j=0;j<38;++j){
emit({randv(0,384),p[j],r});
}}for(int j=0;j<38;++j){
emit({0,0,0});
}}
 std::vector<std::vector<uint64_t>> traces(NS);
 std::vector<std::ofstream> pattern_files;
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/bicg_q_acc.pattern");
pattern_files.emplace_back(pattern_dir+"/bicg_s_acc.pattern");}
