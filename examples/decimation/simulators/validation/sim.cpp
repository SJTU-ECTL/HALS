...
static constexpr int widths[]={16,16,16,16,16}, signs[]={1,1,1,1,1};
...
emit({uint16_t(randv(-16384,16383))});
}
 std::vector<std::vector<uint64_t>> traces(NS);
 std::vector<std::ofstream> pattern_files;
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/decim_stage1.pattern");
pattern_files.emplace_back(pattern_dir+"/decim_stage2.pattern");
pattern_files.emplace_back(pattern_dir+"/decim_stage3.pattern");
pattern_files.emplace_back(pattern_dir+"/decim_stage4.pattern");
pattern_files.emplace_back(pattern_dir+"/hals_decim_stage5.pattern");}
...
dut.mode4=cfg[4].mode;dut.arg4=cfg[4].a;dut.argb4=cfg[4].b;
  ...
  for(const auto&s:stimuli){dut.data_in=s.x[0];
dut.rst=s.reset;
...
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<112;++j) pattern_files[0]<<char('0'+((dut.pi0[j/32] >> (j%32))&1));pattern_files[0]<<"\n";}
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<112;++j) pattern_files[1]<<char('0'+((dut.pi1[j/32] >> (j%32))&1));pattern_files[1]<<"\n";}
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<112;++j) pattern_files[2]<<char('0'+((dut.pi2[j/32] >> (j%32))&1));pattern_files[2]<<"\n";}
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<176;++j) pattern_files[3]<<char('0'+((dut.pi3[j/32] >> (j%32))&1));pattern_files[3]<<"\n";}
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<368;++j) pattern_files[4]<<char('0'+((dut.pi4[j/32] >> (j%32))&1));pattern_files[4]<<"\n";}
++pattern_count;}dut.clk=1;dut.eval();
   if(!s.reset){out.push_back(signvalue(dut.data_out,16,1));}++cycle;
