...
static constexpr int widths[]={16,16,16}, signs[]={0,0,0};
...
emit({0,0,0,0,0},true,false);
for(int p=0;p<npatterns;++p){
emit({randv(0,512),randv(0,512),randv(0,512),384,307});
}
 std::vector<std::vector<uint64_t>> traces(NS);
 std::vector<std::ofstream> pattern_files;
 if(!pattern_dir.empty()){pattern_files.emplace_back(pattern_dir+"/gesummv_acc.pattern");
pattern_files.emplace_back(pattern_dir+"/gesummv_combine.pattern");}
...
dut.mode2=cfg[2].mode;dut.arg2=cfg[2].a;dut.argb2=cfg[2].b;
  ...
  for(const auto&s:stimuli){dut.Aij=s.x[0];
dut.Bij=s.x[1];
dut.xj=s.x[2];
dut.alpha=s.x[3];
dut.beta=s.x[4];
...
if(capture && !pattern_dir.empty() && pattern_count<4096){for(int j=0;j<80;++j) pattern_files[0]<<char('0'+((dut.pi0[j/32] >> (j%32))&1));pattern_files[0]<<"\n";}
...
++pattern_count;}dut.clk=1;dut.eval();
   if(!s.reset && dut.valid_out){out.push_back(signvalue(dut.out,16,0));}++cycle;
